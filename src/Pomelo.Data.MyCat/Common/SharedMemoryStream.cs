...
namespace Pomelo.Data.Common
{
  ...
  internal class SharedMemory : IDisposable
  {
    ...
    public SharedMemory(string name, IntPtr size)
    {
      ...
      {
        throw new MyCatException("Cannot open file mapping " + name);
      }
  ...
  internal class SharedMemoryStream : Stream
  {
    ...
    private void GetConnectNumber(uint timeOut)
    {
          ...
          new SharedMemory(memoryName + "_CONNECT_DATA", (IntPtr)4))
      {
        ...
        if (!connectRequest.Set())
          throw new MyCatException("Failed to open shared memory connection");
        if (!connectAnswer.WaitOne((int)(timeOut * 1000), false))
          throw new MyCatException("Timeout during connection");
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
      ...
      while (bytesLeft == 0)
      {
        ...
        if (waitHandles[index] == connectionClosed)
          throw new MyCatException("Connection to server lost", true, null);
    ...
    public override void Write(byte[] buffer, int offset, int count)
    {
      ...
      while (leftToDo > 0)
      {
        ...
        leftToDo -= bytesToDo;
        if (!clientWrote.Set())
          throw new MyCatException("Writing to shared memory failed");
      }
    }
