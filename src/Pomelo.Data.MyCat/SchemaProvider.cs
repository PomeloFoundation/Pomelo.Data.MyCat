...
namespace Pomelo.Data.MyCat
{
    internal class SchemaProvider
    {
        ...
        public SchemaProvider(MyCatConnection connectionToUse)
        {
        ...
        protected void QuoteDefaultValues(MyCatSchemaCollection schemaCollection)
        {
            if (schemaCollection == null) return;
            if (!schemaCollection.ContainsColumn("COLUMN_DEFAULT")) return;

            foreach (MyCatSchemaRow row in schemaCollection.Rows)
            {
        ...
        private static void ParseColumnRow(MyCatSchemaRow row)
        {
        ...
        public virtual MyCatSchemaCollection GetForeignKeys(string[] restrictions)
        {
        ...
        private static List<string> ParseColumns(MyCatTokenizer tokenizer)
        {
      ...
      sr.Close();
        ...
        protected virtual MyCatSchemaCollection GetSchemaInternal(string collection, string[] restrictions)
        {
            switch (collection)
            {
                // common collections
        ...
        protected MyCatSchemaCollection QueryCollection(string name, string sql)
        {
            MyCatSchemaCollection c = new MyCatSchemaCollection(name);
            MyCatCommand cmd = new MyCatCommand(sql, connection);
